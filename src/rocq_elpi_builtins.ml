...
let abstract__grab_global_env_keep_sigma api thunk = (); (fun state ->
  ...
  Rocq_elpi_HOAS.grab_global_env ~uctx state, result, gls)
...
let grab_global_env__drop_sigma_univs_if_option_is_set options api thunk = (); (fun state ->
  ...
  let uctx, state, result, gls = thunk state in
  match options with
  | { keepunivs = Some false } -> Rocq_elpi_HOAS.grab_global_env_drop_univs_and_sigma state, result, gls
  | _ -> Rocq_elpi_HOAS.grab_global_env ~uctx state, result, gls)
...
let clauses_for_later_interp : _ State.component =
  State.declare_component ~name:"rocq-elpi:clauses_for_later" ~descriptor:interp_state
    ~init:(fun () -> [])
    ~start:(fun x -> x)
    ~pp:(fun fmt l ->
...
let dep1 ?inside sigma gr =
  ...
  | ConstRef cst ->
      ...
      let bo = Global.body_of_constant_body (Library.indirect_accessor[@warning "-3"]) cb in
      let l =
        match bo with
        | Some (e,_,_) -> [ty; e]
        | None -> [ty] in
...
|};
  ...
  LPCode Rocq_elpi_builtins_HOAS.code;
  MLData Rocq_elpi_HOAS.record_field_att;
  MLData Rocq_elpi_HOAS.coercion_status;
     ...
     let loc, args =
       if args = [] then None, args
         ...
         | E.CData loc when API.RawOpaqueData.is_loc loc ->
           Some (Rocq_elpi_utils.to_coq_loc (API.RawOpaqueData.to_loc loc)), args
         | _ -> None, x :: args
     in
...
Note: [ctype "bla"] is an opaque data type and by convention it is written [@bla].|};
    ...
    MLData modpath;
    MLData modtypath;
    MLData Rocq_elpi_builtins_synterp.located;

  MLCode(Pred("coq.locate-all",
    In(id, "Name",
    Out(B.list Rocq_elpi_builtins_synterp.located,  "Located",
  ...
  (fun s _ ~depth ->
    ...
    begin
      ...
      | Some (`Gref gr) -> add @@ Rocq_elpi_builtins_synterp.LocGref gr
      | Some (`Abbrev sd) -> add @@ Rocq_elpi_builtins_synterp.LocAbbreviation sd
      | None -> ()
    end;
    begin
      try add @@ Rocq_elpi_builtins_synterp.LocModule (Nametab.locate_module qualid)
      with Not_found -> ()
    end;
    begin
...
- @uinstance! I (default: fresh instance I)|}))),
  ...
  Rocq_elpi_builtins_synterp.locate_module;
  Rocq_elpi_builtins_synterp.locate_module_type;
...
|};
  ...
  MLCode(Pred("coq.env.end-module",
    Out(modpath, "ModPath",
    Full(unit_ctx, "end the current module that becomes known as ModPath *E*" ^ Rocq_elpi_builtins_synterp.synterp_api_doc)),
  (fun _ ~depth _ _ -> grab_global_env_drop_sigma "coq.env.end-module" (fun state ->
    let state, mp = Rocq_elpi_builtins_synterp.SynterpAction.pop_EndModule () state in
    state, ?: mp, []))),
    DocAbove);
...
|};
  ...
  MLCode(Pred("coq.env.end-module-type",
    ...
    Full(unit_ctx, "end the current module type that becomes known as ModPath *E*" ^ Rocq_elpi_builtins_synterp.synterp_api_doc)),
  (fun _ ~depth _ _ -> grab_global_env_drop_sigma "coq.env.end-module-type" (fun state ->
     let state, mp = Rocq_elpi_builtins_synterp.SynterpAction.pop_EndModuleType () state in
     state, ?: mp, []))),
  DocAbove);
...
declared as cumulative.|};
  ...
  MLCode(Pred("coq.float->float64",
    In(B.float,"F",
    Out(Rocq_elpi_utils.float64,"F64",
  ...
  MLCode(Pred("coq.string->pstring",
    In(B.string,"S",
    Out(Rocq_elpi_utils.pstring,"PS",
...
- @global! (default: false)|})))))),
  ...
  (fun sd arglist _ ~depth {env} _ state ->
    ...
    let binders, vars = List.split (CList.init nargs (fun i ->
      let name = Rocq_elpi_glob_quotation.mk_restricted_name i in
    ...
    let loc = to_coq_loc @@ State.get Rocq_elpi_builtins_synterp.invocation_site_loc state in
    let teta = Rocq_elpi_glob_quotation.runtime_gterm2lp ~loc ~base ~depth geta state in
    let t =
      let rec aux ~depth n t =
        if n = 0 then t
        else match Rocq_elpi_HOAS.is_lam ~depth t with
...
is equivalent to Elpi Export TacName.|})))),
    (fun name tacname more_args ~depth { options} _ -> grab_global_env "coq.notation.add-abbreviation-for-tactic" (fun state ->
      ...
      let tac_fixed_args = more_args |> List.map (function
        | Rocq_elpi_arg_HOAS.Cint n -> Rocq_elpi_arg_HOAS.Tac.Int n
        | Rocq_elpi_arg_HOAS.Cstr s -> Rocq_elpi_arg_HOAS.Tac.String s
        | Rocq_elpi_arg_HOAS.Ctrm t -> Rocq_elpi_arg_HOAS.Tac.Term (Rocq_elpi_utils.detype env sigma t,None)
        | Rocq_elpi_arg_HOAS.CLtac1 _ -> nYI "tactic notation with LTac1 argument") in
  ...
  MLData Rocq_elpi_builtins_synterp.attribute;
...
coq.id->name S N :- coq.string->name S N.
  ...
  Rocq_elpi_builtins_synterp.modtypath_to_library;
...
- @holes! (default: false, prints evars as _)|}))),
  ...
  LPDoc "-- Access to Elpi's data --------------------------------------------";
   ...
   (* Self modification *)
  MLData Rocq_elpi_builtins_synterp.clause;
  MLData Rocq_elpi_builtins_synterp.grafting;
  MLData Rocq_elpi_builtins_synterp.scope;
...
|};
  ...
  MLCode(Pred("coq.elpi.accumulate-clauses",
    In(B.unspec Rocq_elpi_builtins_synterp.scope, "Scope",
    In(id, "DbName",
...
- @global! (default: false, always active, only if Scope is execution-site, discouraged)|} )))),
    (fun scope dbname clauses ~depth {options} _ state ->
      Rocq_elpi_builtins_synterp.accumulate_clauses
...
- @global! (default: false, always active|}))))),
  ...
  ] @ Rocq_elpi_builtins_synterp.SynterpAction.builtins_interp @ [
